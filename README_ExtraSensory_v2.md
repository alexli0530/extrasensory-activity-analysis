...
is associated with the probability of being physically active in the same minute. We use the **ExtraSensory**
smartphone/smartwatch dataset and add contextual information from **NOAA hourly weather data** and a
**2015 non-workday / holiday calendar**.
...
instructions, duplicate setup material, and placeholder headings while preserving the analysis, code, methods,
outputs, and conclusions.

The repository also keeps the **original course files** so that the class submission and team-project context remain
transparent:
...
than when they were alone. After accounting for repeated observations and contextual controls using GEE, the social-context
coefficients were no longer statistically significant. Environmental setting and time of day explained much of the
observed difference.
...
This portfolio repository does **not** include the original raw datasets. The raw ExtraSensory files are relatively large,
and the purpose of this repository is to present the analysis rather than redistribute the full source data.

To make the notebooks easier to run, the repository includes the processed analysis files used by the project under
`data/02-processed/`:
...
These processed files preserve the directory structure expected by the original notebooks, so the main EDA and final-analysis
code can be run without re-downloading and rebuilding the full raw-data pipeline.

### Obtaining the original data

The primary behavioral dataset can be downloaded from the **official UC San Diego ExtraSensory Dataset website**:

https://extrasensory.ucsd.edu/

The project uses the ExtraSensory **features and cleaned context labels** files distributed by the dataset authors.

The raw weather data used in the project come from the **NOAA / NCEI Global Hourly** archive. The specific 2015 station file
used by the course project is:

https://www.ncei.noaa.gov/data/global-hourly/access/2015/72290023188.csv

The non-workday / holiday file in `data/02-processed/` is a derived project file used as a contextual control.

Readers who want to reproduce the full preprocessing pipeline from the original source data can download the ExtraSensory and
NOAA files from the sources above and follow the preprocessing code preserved in the original course notebooks.
